...
export default function CreateEditExam() {
  ...
  if (isLoading) {
    return <div className='min-h-screen flex items-center justify-center text-lg text-gray-600'>Đang tải đề thi...</div>
  }
        ...
        <div className='p-4'>
          ...
          <div className='flex items-center justify-between mb-6'>
            <div className='flex items-center gap-3'>
              <button onClick={() => navigate('/teacher/exams')} className='p-2 hover:bg-gray-100 rounded-lg'>
                <ArrowLeft className='w-4 h-4' />
              </button>

              <div>
                <h1 className='text-xl font-bold text-gray-800'>{isEdit ? 'Chỉnh sửa đề thi' : 'Tạo đề thi mới'}</h1>
          ...
          {questions.map((q, qIndex) => (
            <div key={qIndex} className='mb-6 bg-blue-50 rounded-xl p-4 border border-blue-200 text-sm'>
              <div className='flex justify-between items-center mb-3'>
                ...
                <button onClick={() => removeQuestion(qIndex)} className='text-red-600 hover:text-red-800'>
                  <Trash2 className='w-4 h-4' />
                </button>
              </div>
              ...
              <div className='space-y-2 mb-4'>
                ...
                {q.answers.map((a, aIndex) => (
                  ...
                  >
                    ...
                    {q.answers.length > 2 && (
                      <button onClick={() => removeAnswer(qIndex, aIndex)} className='text-red-500'>
                        <Trash2 className='w-4 h-4' />
                      </button>
                    )}
                  </div>
