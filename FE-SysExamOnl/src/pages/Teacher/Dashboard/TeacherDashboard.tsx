...
export default function TeacherDashboard() {
  ...
  const activeTabKey =
    [...menuItems].sort((a, b) => b.key.length - a.key.length).find((item) => location.pathname.startsWith(item.key))
      ?.key || '/teacher'
  ...
  return (
    <>
      ...
      <div className='min-h-screen flex'>
        ...
        <aside
          className={`${sidebarOpen ? 'w-64' : 'w-30'} transition-all duration-300 ease-in-out bg-white/90 backdrop-blur-2xl shadow-2xl border-r border-white/40 flex flex-col z-20`}
        >
          {/* Logo */}
              ...
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Icon size={sidebarOpen ? 22 : 24} className={isActive ? 'text-indigo-600' : ''} />
