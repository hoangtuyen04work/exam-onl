...
import AIExamGeneratorModal from '../../../../components/AI/AIExamGeneratorModal'
      ...
      <div className='p-3 overflow-hidden'>
                    ...
                    >
                      <div className='flex items-start gap-3 flex-1 min-w-0'>
                        ...
                        <div className='min-w-0 flex-1'>
                          <div className='text-sm font-medium text-slate-800 line-clamp-1 break-words'>{p.name}</div>
                  ...
                  <div className='flex items-start justify-between gap-4'>
                    ...
                    <div className='flex items-center gap-2 flex-shrink-0 flex-wrap'>
                      <button
                        ...
                        className='px-3 py-2 rounded-md bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-700 hover:to-indigo-700 shadow-lg shadow-purple-200 transition'
                      >
                  ...
                  {loadingDetail ? (
                    ...
                    <div className='mt-4 overflow-auto max-h-[44vh] space-y-4'>
                      {selectedPaper.questions.map((q, i) => (
                        <div
                          key={i}
                          className='p-4 bg-gradient-to-r from-slate-50 to-white border border-gray-100 rounded-lg w-full break-words'
                        >
                          <div className='flex justify-between items-start'>
                            <div className='min-w-0 flex-1'>
                              <div className='text-sm text-slate-600'>Câu {i + 1}</div>
                              <h3 className='text-md font-medium text-slate-800 whitespace-pre-wrap break-words'>
                                {q.content}
                              </h3>
                              <div className='text-xs text-slate-500 mt-1'>
                                Độ khó:{' '}
                                <span className='font-medium text-indigo-600'>{difficultyText(q.difficulty)}</span>
                              </div>
                              {q.explanation && (
                  ...
                  {newQuestions.map((q, qIdx) => (
                    <div key={qIdx} className='border border-gray-100 rounded-lg p-4 bg-white'>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewQuestions((prev) =>
                            prev.map((qq, i) => (i === qIdx ? { ...qq, content: e.target.value } : qq))
                          )
                        }
                      />
                      ...
                      <div className='grid grid-cols-1 md:grid-cols-3 gap-3 mt-3'>
                        <select
                          ...
                          onChange={(e) =>
                            setNewQuestions((prev) =>
                              prev.map((qq, i) =>
                                i === qIdx ? { ...qq, difficulty: e.target.value as 'EASY' | 'MEDIUM' | 'HARD' } : qq
                              )
                            )
                          }
                        >
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewQuestions((prev) =>
                              prev.map((qq, i) => (i === qIdx ? { ...qq, point: parseFloat(e.target.value) || 1 } : qq))
                            )
                          }
                        />
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewQuestions((prev) =>
                              prev.map((qq, i) => (i === qIdx ? { ...qq, explanation: e.target.value } : qq))
                            )
                          }
                        />
                      </div>
                                ...
                                setNewQuestions((prev) =>
                                  prev.map((qq, i) =>
                                    i === qIdx
                                      ? {
                                          ...qq,
                                          answers: qq.answers.map((aa, j) =>
                                            j === aIdx ? { ...aa, content: e.target.value } : aa
                                          )
                                        }
                                      : qq
                                  )
        ...
        {showEditModal && editingPaper && (
          <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
            ...
            <div
              className='relative bg-white w-full max-w-4xl rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden flex flex-col border border-gray-100'
              onClick={(e) => e.stopPropagation()}
            >
              <div className='px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-amber-50 to-orange-50'>
                <div>
                  <h3 className='text-lg font-semibold text-slate-800'>✏️ Chỉnh sửa ngân hàng đề</h3>
                  <div className='text-sm text-slate-500'>Cập nhật tên, mô tả và câu hỏi</div>
                </div>
                <button onClick={closeEditModal} className='p-2 rounded-md text-slate-500 hover:bg-slate-50'>
                  <X />
                </button>
              </div>
              ...
              <div className='p-6 overflow-auto space-y-6'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                  <div>
                    ...
                    <input
                      className='w-full border border-gray-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-200 focus:border-amber-400'
                      placeholder='Tên ngân hàng đề'
                      value={newName}
                      onChange={(e) => setNewName(e.target.value)}
                    />
                  ...
                  <div>
                    ...
                    <textarea
                      className='w-full border border-gray-200 rounded-lg p-3 text-sm resize-none focus:ring-2 focus:ring-amber-200 focus:border-amber-400'
                      placeholder='Mô tả (tùy chọn)'
                      rows={2}
                      value={newDesc}
                      onChange={(e) => setNewDesc(e.target.value)}
                ...
                <div className='space-y-4'>
                  <div className='flex items-center justify-between'>
                    <h4 className='text-sm font-medium text-slate-700'>Câu hỏi ({newQuestions.length})</h4>
                    <button
                      onClick={() =>
                        setNewQuestions((prev) => [
                          ...prev,
                      ...
                      className='text-amber-600 hover:underline text-sm flex items-center gap-2'
                    >
                      <Plus size={14} /> Thêm câu hỏi
                    </button>
                  </div>
                  ...
                  {newQuestions.map((q, qIdx) => (
                    <div
                      key={qIdx}
                      className='border border-gray-100 rounded-lg p-4 bg-gradient-to-r from-slate-50 to-white'
                    >
                      <div className='flex justify-between items-start mb-3'>
                        <div className='font-medium text-slate-700'>Câu {qIdx + 1}</div>
                        <div className='flex items-center gap-2'>
                      ...
                      <textarea
                        className='w-full border border-gray-200 rounded-lg p-3 text-sm resize-none focus:ring-2 focus:ring-amber-200'
                        placeholder='Nội dung câu hỏi'
                        rows={2}
                        value={q.content}
                        onChange={(e) =>
              ...
              <div className='px-6 py-4 border-t flex justify-end gap-3 bg-gray-50'>
                ...
                <button
                  ...
                  className='px-5 py-2 rounded-md bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600 shadow-md'
                >
                  💾 Lưu thay đổi
                </button>
              </div>
...
function CreateFromBankButton({ selectedPaper: selectedPaperProp }: { selectedPaper: QuestionPaper | null }) {
  ...
  useEffect(() => {
    if (open && selectedPaperProp) {
      ...
      const counts = (selectedPaperProp.questions || []).reduce(
        (acc, q) => {
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center'>
          ...
          <div className='relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 z-10'>
            <div className='flex items-center justify-between mb-4'>
              ...
              <button onClick={() => setOpen(false)} className='p-2 rounded-md text-slate-500 hover:bg-slate-50'>
                <X />
              </button>
            </div>
            ...
            <div className='space-y-4'>
              <div>
                ...
                <div className='mt-1 p-3 bg-slate-50 rounded text-sm text-slate-700'>
                  {selectedPaperProp?.name ?? '—'}
                </div>
              </div>
